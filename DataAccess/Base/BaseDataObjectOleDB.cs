...
{
	public class BaseDataObjectOleDB
    ...
    #region Constructors
		public BaseDataObjectOleDB() {
			_isShareTxn = false;
			_isShareCxn = false;
		}
		public BaseDataObjectOleDB (IDbConnection pCxn)
		{
		    if (pCxn == null)
		        _isShareCxn = false;
		    else{
		...
		public BaseDataObjectOleDB(IDbTransaction pTxn)
        {
            if (pTxn == null){
                _isShareTxn = false;
				_isShareCxn = false;
		...
		public BaseDataObjectOleDB(IDbConnection pCxn, IDbTransaction pTxn)
		{
		    if (pCxn == null){
		        _isShareCxn = false;
				_isShareTxn = false;
		...
		public static void OpenData(ref IDbConnection cxn){
            try{
				if (cxn != null){
					...
					cxn = new OleDbConnection(WebConfig.ConnectionStringOleDB);
					cxn.Open();
				}
			}catch (Exception ex){
                throw new Exception("Không mở được Database.\n\r" + ex.Message);
		...
		protected void OpenConnection() {
			try{
				if (_isShareCxn){//check exist connection is open
					...
					_cxn = new OleDbConnection(WebConfig.ConnectionStringOleDB);
					_cxn.Open();
				}
			}catch (Exception ex){
	...
	#region CreateParameter Methods
        protected OleDbParameter CreateParameter(string paramName,
            OleDbType paramType, object paramValue)
        {
        ...
        protected OleDbParameter CreateParameter(string paramName, OleDbType paramType, ParameterDirection direction)
        {
            OleDbParameter returnVal = CreateParameter(paramName, paramType, DBNull.Value);
            returnVal.Direction = direction;
            return returnVal;
        }

        protected OleDbParameter CreateParameter(string paramName, OleDbType paramType, object paramValue, ParameterDirection direction)
        {
            OleDbParameter returnVal = CreateParameter(paramName, paramType, paramValue);
            returnVal.Direction = direction;
            return returnVal;
        }

        protected OleDbParameter CreateParameter(string paramName, OleDbType paramType, object paramValue, int size)
        {
            OleDbParameter returnVal = CreateParameter(paramName, paramType, paramValue);
            returnVal.Size = size;
            return returnVal;
        ...
        protected OleDbParameter CreateParameter(string paramName, OleDbType paramType, object paramValue, int size, ParameterDirection direction)
        {
            OleDbParameter returnVal = CreateParameter(paramName, paramType, paramValue);
            returnVal.Direction = direction;
            returnVal.Size = size;
        ...
        protected OleDbParameter CreateParameter(string paramName, OleDbType paramType, object paramValue, int size, byte precision)
        {
            OleDbParameter returnVal = CreateParameter(paramName, paramType, paramValue);
            returnVal.Size = size;
            ((OleDbParameter)returnVal).Precision = precision;
            return returnVal;
        }

        protected OleDbParameter CreateParameter(string paramName, OleDbType paramType, object paramValue, int size, byte precision, ParameterDirection direction)
        {
            OleDbParameter returnVal = CreateParameter(paramName, paramType, paramValue);
            returnVal.Direction = direction;
            returnVal.Size = size;
	...
	#region ExecuteDataSet Methods
		...
		private DataSet ExecuteDataSet(out OleDbCommand pcmdData, string procName, params IDataParameter[] procParams)
        {
            DataSet ldsData = null;
            OleDbDataAdapter ldaData = null;
			int liParaCount;
            pcmdData = null;
            try{
                //Setup command object
                pcmdData = new OleDbCommand(procName);
				...
				if (_isShareTxn){
					pcmdData.Connection = (OleDbConnection)_txn.Connection;
                    pcmdData.Transaction = (OleDbTransaction)_txn;
				}else{
					OpenConnection();
					pcmdData.Connection = (OleDbConnection)_cxn;
                }
                //Fill the dataset
				ldsData = new DataSet();
				ldaData = new OleDbDataAdapter(pcmdData);
    ...
    #region ExecuteDataTable Methods
		...
		private DataTable ExecuteDataTable(out OleDbCommand pcmdData, string procName, params IDataParameter[] procParams)
        {
	...
	#region ExecuteNonQuery Methods
		...
		/// </summary>
		public int ExecuteNonQuery(OleDbCommand pcmdData)
        {
			int liRes = 0;
            try{
                //Execute the command
		...
		private int ExecuteNonQuery(out OleDbCommand pcmdData, string procName, params IDataParameter[] procParams)
        {
            //Method variables
    ...
    #region CreateCommand Methods
		...
		/// Tạo command
		/// </summary>
		public OleDbCommand CreateCommand(string procName, params IDataParameter[] procParams)
        {
            OleDbCommand lcmdData = null;  //Avoids "Use of unassigned variable" compiler error
			int liParaCount;
            try{
                //Setup command object
                lcmdData = new OleDbCommand(procName);
                ...
                //Determine the transaction owner and process accordingly
				if (_isShareTxn){
					lcmdData.Connection = (OleDbConnection)_txn.Connection;
                    lcmdData.Transaction = (OleDbTransaction)_txn;
				}else{
					OpenConnection();
					lcmdData.Connection = (OleDbConnection)_cxn;
                }
            }catch{
                throw;
			}
