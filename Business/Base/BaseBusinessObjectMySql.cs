...
{
	public class BaseBusinessObjectMySql : BaseGets
	{
	#region Attributes
	...
	#region Connection and Transaction New Methods
		public static void OpenData(ref IDbConnection cxn){
			DataAccess.BaseDataObjectMySql.OpenData(ref cxn);
		}
		public static void CloseConnect(ref IDbConnection cxn){
			DataAccess.BaseDataObjectMySql.CloseConnect(ref cxn);
        }
		public static void BeginTransaction(ref IDbConnection cxn, ref IDbTransaction txn)
		{
			txn = DataAccess.BaseDataObjectMySql.BeginTransaction(ref cxn);
		}
		public static void Commit(ref IDbTransaction txn){
			try{
				DataAccess.BaseDataObjectMySql.Commit(ref txn);
			}catch(Exception ex){
		...
		public static void Rollback(ref IDbTransaction txn){
			try{
				DataAccess.BaseDataObjectMySql.Rollback(ref txn);
			}catch(Exception ex){
				throw new Exception("[Rollback] " + ex.Message);
			}
        }
		public void OpenData(){
			DataAccess.BaseDataObjectMySql.OpenData(ref _cxn);
		}
		
		public void CloseConnect(){
			DataAccess.BaseDataObjectMySql.CloseConnect(ref _cxn, ref _txn);
        }
		/// <summary>
		...
		{
			_txn = DataAccess.BaseDataObjectMySql.BeginTransaction(ref _cxn);
		}
		public void Commit(){
			try{
				DataAccess.BaseDataObjectMySql.Commit(ref _txn);
			}catch(Exception ex){
				throw new Exception("[Commit] " + ex.Message);
			}
        }
		public void Rollback(){
			try{
				DataAccess.BaseDataObjectMySql.Rollback(ref _txn);
			}catch(Exception ex){
				throw new Exception("[Rollback] " + ex.Message);
			}
        }
